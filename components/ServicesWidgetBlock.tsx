...
const ServicesWidgetBlock = () => {
    ...
    const getCat = async (slug: string) => {
        try {
            ...
            const res = await fetch(`${WP_URL}/wp-json/wp/v2/posts?_embed&per_page=1&categories=${catId}`);
    ...
    useEffect(() => {
        const load = async () => {
            const [g, a, s] = await Promise.all([
                getCat('backstage-geek'),
                getCat('eventos'),
                getCat('sertanejo-e-meio')
            ]);
            setGamePost(g[0] || null);
            setEventPost(a[0] || null);
            setSertaPost(s[0] || null);
        };
        load();
    }, [WP_URL]);
        ...
        <div className="bg-white border border-gray-100 rounded-lg p-5 shadow-sm hover:shadow-md transition-shadow h-full flex flex-col">
            ...
            {post ? (
                <div className="flex flex-col gap-3 flex-1">
                    ...
                    <h4 className="text-[13px] font-bold text-gray-800 leading-snug line-clamp-3" dangerouslySetInnerHTML={{ __html: post.title.rendered }} />
                </div>
            ) : (
    ...
    return (
        <section className="w-full max-w-[1280px] mx-auto px-4 py-4 mb-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <WidgetCard title="Games & Geek" color="#9333ea" post={gamePost} label="DESTAQUE GEEK" />
                <WidgetCard title="Agenda Cultural" color="#10b981" post={eventPost} label="PRÓXIMOS EVENTOS" />
                <WidgetCard title="Sertanejo & 1/2" color="#f97316" post={sertaPost} label="MUNDO DA MÚSICA" />
            </div>
        </section>
    );
};
