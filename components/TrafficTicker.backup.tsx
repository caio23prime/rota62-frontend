...
export default function TrafficTicker() {
  ...
  return (
    // AJUSTE: h-16 (mobile) / h-20 (desktop)
    <div className="w-full h-16 md:h-20 bg-black/95 flex items-center overflow-hidden relative border-y border-gray-800 font-sans mt-0 mb-4 z-50 shadow-lg">
      
      {/* CABEÇALHO LATERAL FIXO: Reduzido para w-20 no mobile */}
      <div className="bg-red-700 h-full w-20 md:w-36 flex items-center justify-center shrink-0 z-20 relative shadow-[5px_0_20px_rgba(0,0,0,0.8)] border-r border-red-900">
        <div className="flex flex-col items-center justify-center leading-tight">
            <div className="flex flex-col items-center gap-1">
                <span className="relative flex h-2 w-2 md:h-2.5 md:w-2.5 mb-0.5">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-full w-full bg-white"></span>
                </span>
                <span className="text-white font-black text-[9px] md:text-[10px] lg:text-xs tracking-wide text-center leading-none">
                    TRÂNSITO<br/>AO VIVO
                </span>
            </div>
        </div>
      ...
      {/* ÁREA DE CONTEÚDO */}
      <div className="flex-1 h-full flex items-center relative overflow-hidden bg-gray-900/50 px-2 md:px-6">
        ...
        <div key={currentIndex} className="animate-slide-up flex items-center w-full z-10 h-full">
            ...
            {/* ÍCONE: Reduzido para mobile (w-8), mantém 38px no desktop via classes */}
            <div className={`mr-2 md:mr-4 shrink-0 ${(item.corTitulo || "").includes('red') ? 'animate-pulse' : ''}`}>
                <item.Icon className="w-8 h-8 md:w-[38px] md:h-[38px]" strokeWidth={1.5} {...item.iconProps} />
            </div>

            {/* CONTAINER FLEXÍVEL: Coluna no mobile, Linha no Desktop */}
            <div className="flex flex-col md:flex-row md:items-center justify-center items-start gap-0.5 md:gap-3 w-full h-full md:content-center md:flex-wrap">
                
                {/* BLOCO 1 (Título e Tags) */}
                <div className="flex items-center gap-2 shrink-0 h-auto md:h-7">
                    <span className={`text-xs md:text-sm font-black uppercase tracking-widest ${item.corTitulo} whitespace-nowrap`}>
                        {item.titulo}
                    </span>
                    
                    <span className={`${item.corOrigem} text-white text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm uppercase tracking-wide whitespace-nowrap`}>
                        {item.origem}
                    </span>

                    {item.atraso && (
                        <span className="bg-red-600/90 text-white text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm animate-pulse whitespace-nowrap">
                            {item.atraso}
                        </span>
                    )}
                </div>

                {/* BLOCO 2 (Texto): Menor no mobile, remove min-width fixo no mobile */}
                <span className="text-xs md:text-xl font-bold text-white leading-tight flex-1 min-w-0 md:min-w-[280px] line-clamp-2 md:line-clamp-none">
            ...
            {/* CONTADOR DE SLIDES */}
            <div className="absolute right-1 md:right-2 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-30">
                {items.map((_, idx) => (
                    <div 
                        key={idx} 
